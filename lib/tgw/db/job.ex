defmodule Tgw.Db.Job do
  ...
  def mark_timedout(job),
    do: Tgw.Repo.update(changeset(job, %{status: :failed, error: "timed out"}))

  def in_flight do
    q =
      from j in Tgw.Db.Job,
        left_join: w in Tgw.Db.Worker,
        on: j.worker_id == w.id,
        left_join: t in Tgw.Db.Task,
        on: j.task_id == t.id,
